+function ($) {
	...
	var Check = function (element, options) {
		...
		this.label = typeof options.label != 'undefined' ? options.label : this.$element.attr('data-label')
		...
		if (typeof options.ajax == 'undefined') {
			...
			this.$element.on('change', function () {
				if (check() !== false) {
					$.get(options.ajax.url, { 'value': that.$element.val() }, function (data) {
						if (data.code === 0 && data.exists === 1) {
							error(options.ajax.message)
						}
					}, 'json');
				}
			})
