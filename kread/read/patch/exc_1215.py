...
def _upgrade_article(article):
	xcontent = iweb.text_file.get(article['content']).decode('utf-8')
	...
	if words is not None:
		...
		iweb.word_file.put(article['_id'], json.dumps(words))
		...
		content = web_content(xcontent, article, words)
...
def upgrade():
	doc = {'v.seg':0.1}
	articles = iweb.spider_article.find(doc, timeout=False)
	count = 0
	for article in articles:
		upgrade_article(article)
		count += 1
		if count % 100 == 0:
			print count
